...
def delete_connections(locations_list, location_id):
    for loc_id in locations_list.keys():
        if loc_id == location_id:
            continue
        cells = locations_list[loc_id]['cells']
...
def read_locations():
    ...
    if os.path.isfile(LOCATIONS_FILE) and os.path.getsize(LOCATIONS_FILE) > 0:
        # If the file exists and is not empty, read its contents into a dictionary
        with open(LOCATIONS_FILE, 'r') as f:
            locations = eval(f.read())
        return locations
    else:
        locations = {0:{'name':'START','description':'You can edit it','cells':{},}}
        return locations
...
def add_location():
    ...
    if not locations:
        return print("No loacations inside")
    ...
    if len(locations) <= 1:
        ...
        for key in locations:
            loc_IDs.append(key)
            location_id = max(loc_IDs)
        print("ID's in use: ", loc_IDs )
